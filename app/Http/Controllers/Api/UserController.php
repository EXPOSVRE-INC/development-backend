...
class UserController extends Controller
{
    ...
    {
        $interest = InterestsCategory::where(['name' => $request->get('interest')])->first();
        auth('api')->user()->assignInterest($interest->id);
        return InterestsResource::collection(auth('api')->user()->interests);
    }
...
//        dd($userInterests);
        ...
        auth('api')->user()->revokeInterests();

        if (is_array($interests)) {
            foreach ($interests as $interest) {
                if ($interest) {
                    $findInterest = InterestsCategory::where(['name' => $interest])->first();
                    auth('api')->user()->assignInterest($findInterest->id);
                }
            }
        }
    ...
    public function assignNotInterest(Request $request)
    {
        ...
        auth('api')->user()->assignNotInterest($interest->id);
    ...
    {
        $interests = $request->get('interests');
        auth('api')->user()->revokeNotInterests();

        if (is_array($interests)) {
            foreach ($interests as $interest) {
                if ($interest) {
                    $findInterest = InterestsCategory::where(['name' => $interest])->first();
                    auth('api')->user()->assignNotInterest($findInterest->id);
                }
            }
        }
        auth('api')->user()->refresh();
    ...
    public function feed(FeedRequest $request)
    {
        ...
        $userInterestsArray = $userInterests->map(function ($interest) {
...
//        \DB::enableQueryLog();
        $postsInterested = Post::with('interests')->whereHas('interests', function ($query) use ($userInterestsArray) {
...
//        })->toArray();
        ...
        foreach ($user->subscriptions as $subscriptionUser) {
            $newPosts = $subscriptionUser->posts->filter(function ($post) {
                return $post->reports->count() == 0;
            })
//                ->filter(function ($post) use ($fromDate) {
//                    return $post->updated_at >= $fromDate;
...
//        };
        ...
        $postsInterested = count($postsInterested) > 0 ? array_values($postsInterested->diff($posts)->toArray()) : [];

        rsort($postsInterested);
//        dump($postsInterested);
//        dump($posts);

        $newArray = $posts->chunk(2)->map(function($items, $key) use ($postsInterested) {
...
//            $geocodeFromAddr = file_get_contents('https://maps.googleapis.com/maps/api/geocode/json?address=' . $formattedAddr . '&sensor=false&key=' . $apiKey);
        $geocodeFromAddr = file_get_contents('https://maps.googleapis.com/maps/api/place/autocomplete/json?input=' . $formattedAddr . '&types=geocode&key=' . $apiKey);
    ...
    {
        return response()->json(['data' => new UserMilestoneResource(auth('api')->user())]);
    }
...
//        dump($message);
    ...
    public function marketLike($id) {
        ...
        $like = auth('api')->user()->like($profile);
    ...
    public function marketUnlike($id) {
        $user = User::where(['id' => $id])->first();
        $profile = $user->profile;
        $like = auth('api')->user()->unlike($profile);
        return response()->json(['data' => $like]);
    }
    ...
    public function blockUser($blockUserID)
    {
        ...
        return response()->json(['data' => 'You are blocked user ' . $blockUser->username]);
    }
...
//        dd($users);
        ...
        return response()->json(['data' => UserInfoResource::collection($users)]);
    }
    ...
    public function unblockUser($blockUserID)
    {
        ...
        return response()->json(['data' => 'You are unblocked user ' . $blockUser->username]);
    }
}
