                    ...
                    {{$post->owner->profile->firstName . ' ' . $post->owner->profile->lastName}}
                                ...
                                <td>
                                    <div class="btn-group btn-group-sm">
                                        <a class="btn btn-sm btn-primary" href="{{Route::is('ads-archive') ? route('ads-from-archive', $post->id) : route('ads-to-archive', $post->id) }}">
                                            <i class="fa fa-lg fa-fw fa-archive"></i>{{Route::is('ads-archive') ? "From archive" : "To Archive"}} </a>
                    {{--                    <button class="btn btn-xs btn-default text-danger mx-1 shadow" title="Delete">--}}
                    {{--                        <i class="fa fa-lg fa-fw fa-trash"></i>--}}
                    {{--                    </button>--}}
                                        <a class="btn btn-sm btn-default text-teal shadow" title="Details" href="{{route('ads-edit', ['id' => $post->id])}}">
                                            <i class="fa fa-lg fa-fw fa-eye"></i>Edit
                                        </a>
                                        <a class="btn btn-sm btn-default text-teal shadow" title="Move to top" href="{{route('prioritise-post', ['id' => $post->id])}}">
                                            <i class="fa fa-lg fa-fw fa-eye"></i>Move to top
                                        </a>
                                        <a class="btn btn-sm btn-danger shadow" title="Remove" href="{{route('post-delete', ['id' => $post->id])}}">
